...
public class AfiliadoDAO {
	...
	public void getAfiliado(Afiliado Afiliado) {
		...
		try {
			...
			while (rset.next()) {
				...
				afiliado.getEndereco().setIdEndereco(rset.getInt("Id_endere�o"));
	...
	public Afiliado pegarAfiliado(int id) {
		...
		sql += "Id_endere�o, CEP, Estado, Bairro, Cidade, Numero, Logradouro ";
		...
		/*String sql = "SELECT cooperativa.Id_usu�rio, Nome, CPF, Email, Senha, Telefone, Celular, ";
		sql += "Id_endere�o, CEP, Estado, Bairro, Cidade, Numero, Logradouro ";
		sql += "FROM cooperativa INNER join endereco ON cooperativa.Id_usu�rio = endereco.Id_usu�rio ";
		sql += "WHERE cooperativa.id_usu�rio = ? ";
		...
		try {
			...
			//SELECT Id_usu�rio, Nome, CPF, Email, Senha, Telefone, Celular FROM cooperativa WHERE 1
				afiliado.setIdCooperativa(rset.getInt("Id_cooperativa"));
				afiliado.setNome(rset.getString("Nome"));
				afiliado.setEmail(rset.getString("Email"));
				afiliado.setSenha(rset.getString("Senha"));
